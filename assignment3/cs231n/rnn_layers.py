...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    outx, cachex = affine_forward(x, Wx, b/2)
    outh, cacheh = affine_forward(prev_h, Wh, b/2)
    next_h = np.tanh(outx + outh)
    cache_tanh = next_h
    cache = [cachex, cacheh, cache_tanh]
...
def rnn_step_backward(dnext_h, cache):
    ...
    cachex, cacheh, cache_tanh = cache
    d_before_tanh = dnext_h * (1-cache_tanh*cache_tanh) # [N, H]
    d_outx = d_before_tanh # [N, H]
    d_outh = d_before_tanh # [N, H]
    dx, dWx, db1 = affine_backward(d_outx, cachex)
    dprev_h, dWh, db2 = affine_backward(d_outh, cacheh)
    db = (db1+db2)/2
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if i == 0:
            prev_h, prev_cache = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)#prev_h: [N, H]
        else:
            prev_h, prev_cache = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
        h.append(prev_h[:, np.newaxis, :])# [N, 1, H]
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == T-1:
            ...
            dx_i, dprev_h, dWx_i, dWh_i, db_i = rnn_step_backward(dh[:, i, :]+dprev_h, cache[i])
            dx.append(dx_i[:, np.newaxis, :])
            dWx += dWx_i
            dWh += dWh_i
            db += db_i
    dh0 = dprev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, H = prev_h.shape[0], prev_h.shape[1]
    affine_x, cache_x = affine_forward(x, Wx, b/2) # [N, 4H]
    affine_h, cache_h = affine_forward(prev_h, Wh, b/2) # [N, 4H]
    ifog = affine_x + affine_h # [N, 4H]
    ifog = ifog.reshape(N, 4, H)
    i = sigmoid(ifog[:, 0, :])
    f = sigmoid(ifog[:, 1, :])
    o = sigmoid(ifog[:, 2, :])
    g = np.tanh(ifog[:, 3, :])
    ...
    cache_lstm = (i, f, o, g, next_c, prev_c)
    cache = [cache_x, cache_h, cache_lstm]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    cache_x, cache_h, cache_lstm = cache
    i, f, o, g, next_c, prev_c = cache_lstm
    dnext_c_tanh = o * dnext_h # [N, H]
    dnext_c += dnext_c_tanh * (1-np.tanh(next_c) * np.tanh(next_c))
    ...
    do = do * o * (1-o)
    dg = dg * (1-g*g)
    difog = np.concatenate([di, df, do, dg], axis=1) # [N, 4H]
    dx, dWx, db1 = affine_backward(difog, cache_x)
    dprev_h, dWh, db2 = affine_backward(difog, cache_h)
    db = (db1+db2)/2
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if i == 0:
            nexth, nextc, next_cache = lstm_step_forward(x[:, i,:], h0, c0, Wx, Wh, b)
        else:
            nexth, nextc, next_cache = lstm_step_forward(x[:, i, :], nexth, nextc, Wx, Wh, b)
        h.append(nexth[:, np.newaxis, :])
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        if i == T-1:
            dxi, dprev_h, dprev_c, dWx, dWh, db = lstm_step_backward(dh[:, i, :], np.zeros_like(dh[:,i,:]),cache[i])
        else:
    ...
    dx = np.concatenate(dx[::-1], axis=1) # [N ,T, H]
    dh0 = dprev_h
