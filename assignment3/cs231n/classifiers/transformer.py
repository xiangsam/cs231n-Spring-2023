...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        input_embed = self.visual_projection(features).unsqueeze(1) # [N, 1, W]
        tgt_mask = torch.tril(torch.ones(T, T, dtype=caption_embed.dtype, device=caption_embed.device)) # [T, T]
        output = self.transformer(caption_embed, input_embed, tgt_mask) # [N, T, W]
